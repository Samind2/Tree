...
function createTreeElement(item) {
  ...
  if (item.type !== "root") {
    ...
    deleteButton.className = "delete-button"; // เพิ่มคลาส delete-button
...
function createTreeElement(item) {
  ...
  if (item.type === "folder") {
    icon.className = "fas fa-folder";
    // Add event listener to toggle visibility of children
    icon.addEventListener("click", function () {
      const children = element.querySelectorAll(".file, .folder");
...
function initializeFolderStructure(structure) {
  structure.showChildren = true; // Initially show children
  if (structure.children) {
    structure.children.forEach((child) => {
      initializeFolderStructure(child);
    });
  }
}

// Call initializeFolderStructure with the root structure
initializeFolderStructure(folderStructure);
